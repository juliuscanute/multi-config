...
public class ConfigurationDetailController: NiblessViewController {
    ...
    private func showEditableDialog(editableState: EditableState) {
        let alert = UIAlertController(title: AppString.edit_configuration, message: editableState.description, preferredStyle: UIAlertController.Style.alert)
        alert.addTextField { (inputText) in
            inputText.text = editableState.currentValue
        }
        alert.addAction(UIAlertAction(title: AppString.ok, style: .default, handler: { [weak alert, weak self] (_) in
        ...
        alert.addAction(UIAlertAction(title: AppString.cancel, style: UIAlertAction.Style.cancel, handler: nil))
        self.present(alert, animated: true, completion: nil)
    }

    private func showChoiceDialog(choiceState: ChoiceState) {
        let alert = UIAlertController(title: AppString.choose_configuration, message: choiceState.description, preferredStyle: UIAlertController.Style.alert)
